...
sudo tee /etc/apt/sources.list.d/docker.list <<< "deb https://apt.dockerproject.org/repo ubuntu-$(lsb_release -cs) main"

# Install Docker, Vagrant, dnsmasq
sudo apt-get update
sudo apt-get install python-ipaddress
sudo apt-get install -y docker-engine linux-image-extra-virtual dnsmasq ruby-dev
